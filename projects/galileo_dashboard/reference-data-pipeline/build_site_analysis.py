...
For each site (db.json column 1) it aggregates, over the current YTD period and
the same period last year: dominant geo + all geos touched, the products and
site types it handles, and pieces + shipments split by market (REP / LM). That
is enough for a single-site card (volume, shipments, batch size, all vs PY)
without shipping the heavy db.json to the Content V2 route.

Output: app-next/src/data/site_analysis.json (~50 KB). Run after build_content.py:
    python scripts/build_site_analysis.py
"""
...
def sheet1_key_for(product, site_type):
    ...
    Same mapping as build_content / build_content_trends."""
...
def main():
    ...
    def new_site():
        return {
            "areas": defaultdict(float),  # geo -> pieces (to pick dominant)
            "products": set(),
            "site_types": set(),
            # market -> {pieces_cur, pieces_py, ship_cur, ship_py}
            "rep": [0.0, 0.0, 0.0, 0.0],
            "lm": [0.0, 0.0, 0.0, 0.0],
        }

    sites = defaultdict(new_site)
    # flow -> area -> set(sites) : which plants feed each Content flow, by area.
    # Drives the comment site picker so only sites belonging to the section
    # (flow + area) being commented on are taggable. "ALL" = union across areas.
    flow_sites = defaultdict(lambda: defaultdict(set))
    # period -> flow -> area -> site -> [repPcur, repPpy, repScur, repSpy,
    #                                    lmPcur,  lmPpy,  lmScur,  lmSpy]
    # Per-site figures WITHIN a flow+area (effective geo, col 10 — same
    # attribution as content.json cells, Export Labs APAC→EMEA included), so
    # the metric explorer can name the top driver plants behind an area's
    # number. Keyed by cumulative end month (1..period) so the drill stays
    # consistent with the Content month selector. "ALL" = across areas.
    fsm = defaultdict(
    ...
    for r in db:
        ...
        # Flow membership: real activity only (skip 0/0 rows).
        if (pieces > 0 or shipments > 0) and geo:
            ...
            if flow is not None:
                ...
                # A month contributes to every cumulative end month >= its own.
                for p in PERIODS:
                    if mn > p:
                        continue
