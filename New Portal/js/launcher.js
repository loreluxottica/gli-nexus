...
(function () {
  ...
  /* Badge di stato: letto da meta ("Stato") della scheda detail. */
  function statusOf(id) {
    const det = detailOf(id);
    if (!det || !det.meta) return null;
    const m = det.meta.find(x => x.label === "Stato");
  ...
  groups.forEach(group => {
    ...
    group.items.forEach(product => {
      ...
      tile.innerHTML = `
        ...
        <span class="ltile-open" aria-hidden="true">Apri
  ...
  countEl.innerHTML = "<b>" + String(NEXUS_WORLDS.length).padStart(2, "0") + "</b> prodotti";
  ...
  function applyFilter() {
    ...
    countEl.innerHTML = q
      ? "<b>" + String(shown).padStart(2, "0") + "</b> su " + String(tiles.length).padStart(2, "0")
      : "<b>" + String(tiles.length).padStart(2, "0") + "</b> prodotti";
  }
